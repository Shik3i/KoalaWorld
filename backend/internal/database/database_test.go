...
func TestUpsertAndGetEvents(t *testing.T) {
	...
	// Read back
	events, err := db.GetEvents("earthquake", 10, 0, "", "", nil, "", "", "")
	if err != nil {
	...
	events, _ = db.GetEvents("earthquake", 10, 0, "", "", nil, "", "", "")
	if len(events) != 1 {
...
func TestGetEventsFilters(t *testing.T) {
	...
	// Test type filter
	eqs, _ := db.GetEvents("earthquake", 100, 0, "", "", nil, "", "", "")
	if len(eqs) != 3 {
		t.Errorf("expected 3 earthquakes, got %d", len(eqs))
	}

	// Test limit
	limited, _ := db.GetEvents("", 2, 0, "", "", nil, "", "", "")
	if len(limited) > 2 {
...
func TestBatchUpsertEvents(t *testing.T) {
	...
	result, _ := db.GetEvents("earthquake", 100, 0, "", "", nil, "", "", "")
	if len(result) != 2 {
